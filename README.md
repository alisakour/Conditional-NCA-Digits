# Conditional Neural Cellular Automata for Class-Specific Morphogenesis

[![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1PQas3_64Nx__nJt_HWJZwfcZZrk1o9HJ?usp=sharing))

## Project Demo
Here is the model growing specific digits from a single pixel based on the class condition:

![Morphogenesis Demo](growth_of_digit_8.gif)
*(Example: Growing digit '8' using conditional vectors)*

## Abstract
This repository contains the implementation of the paper **"Conditional Neural Cellular Automata for Class-Specific Morphogenesis of Handwritten Digits"**.
The model extends the original NCA framework to generate specific MNIST digits based on a conditional input vector.

## Author
**Ali Sakour**
Lattakia University, Syria

## Acknowledgements
This work is built upon the "Growing Neural Cellular Automata" paper by Mordvintsev et al.
